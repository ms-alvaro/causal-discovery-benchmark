...
Definition of causality:
    If Q_j causally influences Q_i, Takens' theorem implies that the shadow
    manifold M_i (delay embedding of Q_i) retains information about Q_j.
    CCM_{j→i} = corr(Q_j, Q̂_j|M_i), where Q̂_j is reconstructed from the
    E+1 nearest neighbours of M_i using exponential distance weights.

Reference:
    Sugihara, G. et al., Science 338(6106):496-500 (2012).
    Takens, F., Lecture Notes in Mathematics 898:366-381 (1981).
    As described in Supplementary S2.2 of:
...
from _ccm.ccm_core import ccm_pairwise  # noqa: E402

NAME       = "CCM"
DEFINITION = (
    "Cross-maps Q_j from shadow manifold M_i (delay embedding of Q_i); "
    "CCM_{j→i} = corr(Q_j, Q̂_j|M_i) via E+1 nearest-neighbour reconstruction."
)
REFERENCE  = (
    "Sugihara et al., Science 338:496 (2012); "
    "Takens, Lecture Notes Math. 898:366 (1981); "
    "Martínez-Sánchez, Arranz & Lozano-Durán, Nat. Commun. 15, 9296 (2024). "
    "https://doi.org/10.1038/s41467-024-53373-4"
)
...
_MISS_THR      = 0.05
_SPURIOUS_THR  = 0.25
_ABS_THR       = 0.05   # CCM values in [0,1]; anything < 0.05 is negligible


def run(X: np.ndarray, nbins: int = 50, nlag: int = 1) -> list:
    ccm_matrix = ccm_pairwise(X, nlag=nlag)
    nvars = X.shape[0]
    return [{"ccm_row": ccm_matrix[i], "nvars": nvars} for i in range(nvars)]


def _rel_scores(row: np.ndarray) -> np.ndarray:
    ...
    for c_idx, case_id in enumerate(case_ids):
        ...
        for v_idx, res in enumerate(results):
            ...
            for j in range(nvars):
                ...
                is_missing = float(row[j]) <= _ABS_THR or float(rel[j]) <= _MISS_THR
            ...
            ax.set_title(
                f"$\\mathrm{{CCM}}_{{(\\cdot)\\rightarrow Q_{v_idx+1}^+}}$",
                fontsize=20, pad=16)
...
def evaluate(results: list, case: int) -> dict:
    ...
    Pass criteria (Q1 primary, spurious check all targets):
      Case 1 — Mediator:    Q2 dominates CCM→Q1
      Case 2 — Confounder:  Q3 dominates CCM→Q1 (CCM may detect spurious Q2↔Q1)
      Case 3 — Synergistic: both Q2 and Q3 detected
      Case 4 — Redundant:   both Q2 and Q3 detected
    """
    _CRITERIA = {
        1: (1, "Q2 dominates CCM→Q1 (direct driver in mediator chain)"),
        2: (2, "Q3 dominates CCM→Q1 (common cause; CCM may flag spurious Q2↔Q1)"),
        3: (None, "Both Q2 and Q3 detected via manifold reconstruction"),
